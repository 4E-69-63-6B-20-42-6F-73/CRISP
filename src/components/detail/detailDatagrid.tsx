...
const DetailDataGrid: React.FC<{ items: Prediction[] }> = ({ items }) => {
  const columns: TableColumnDefinition<Prediction>[] = [
    createTableColumn<Prediction>({
      columnId: "PatientId",
      compare: (a, b) => a.patientId.toString().localeCompare(b.patientId.toString()), // Make sure it's a string
      renderHeaderCell: () => "Patient Id",
      renderCell: (item) => <TableCellLayout>{item.patientId}</TableCellLayout>,
    }),
    createTableColumn<Prediction>({
      columnId: "Prediction",
      compare: (a, b) => a.prediction - b.prediction, // Compare as numbers
      renderHeaderCell: () => "Prediction",
      renderCell: (item) => <TableCellLayout>{item.prediction}</TableCellLayout>,
    }),
  ];
