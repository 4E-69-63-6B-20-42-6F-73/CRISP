...
export default function Reorder() {
  ...
  const [order, setOrder] = useState<string[]>(getInitialOrder(oldColumns, expectedColumnInFile))
  ...
  return (
    <>
      <div>
        <h2>Mapping</h2>
        <p>Your data is missing some required column names that our model expects. Please click the buttons in the order that matches your data to assign the correct column names</p>
      </div>
      ...
      <div>
        ...
        <TagGroup>
          {
            numeric.map(x =>
              <ToggableTag text={x} isSelected={order.includes(x)} order={order.findIndex(y => y === x) + 1} key={x} onClick={() => { toggle(x) }} />)
          }
        </TagGroup>
      ...
      <div style={{ display: "flex", flexDirection: "row", gap: "12px" }}>
        <div>
          <h2>Swelling</h2>
          <ClickableMannequin order={order} setOrder={setOrder} id_prefix="Zwelling" />
        </div>

        <div>
          <h2>Pain</h2>
          <ClickableMannequin order={order} setOrder={setOrder} id_prefix="Pijn" />
        </div>
      </div>
      <div>
        <h2>Data</h2>
                ...
                {data.map((record, rowIndex) => (
                  <tr key={rowIndex}>
                    ...
                    {Object.values(record).map((value, colIndex) => (
                      <td key={colIndex}>{value as string}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
